...
// MACD with Stochastic Slow
// Join with Price above sma20
// with Stochastic value below 30
//      
using System;
...
namespace strategy
{
    ...
    public class MACD_Stochastic_Bottom_v1 : GenericStrategy
    {
        ...
        public MACD_Stochastic_Bottom_v1(analysis.workData d, string code, bool fExport, string curStrategy, Parameters p) : base(d, code, fExport, curStrategy, p) { }
        public override analysis.analysisResult Execute()
        {
            ...
            for (int idx = 1; idx < macd.Length; idx++)
            {
                ...
                if (!is_bought && data.closePrice[idx] > sma20[idx])
                    if (macd_swicth_to_up_trend && stochasticTrend == analysis.tradeActions.Sell)
                    {
                        is_bought = true;
