...
namespace Indicators
{
    ...
    public class MIDPRICEHelper : Helpers
    {
        ...
        {
            Init(typeof(MIDPRICE), typeof(forms.commonForm), typeof(DataBars));
        }
    }
    ...
    public class MIDPRICE : DataSeries
    {
        ...
        /// <returns></returns>
        public static MIDPRICE Series(DataBars ds, int period, string name)
        {
            //Build description
        ...
        /// <param name="name"></param>
        public MIDPRICE(DataBars db, int period, string name)
            : base(db, name)
        {
            ...
            retCode = Core.MidPrice(0, db.Count - 1, db.High.Values, db.Low.Values,period, out begin, out length, output);
