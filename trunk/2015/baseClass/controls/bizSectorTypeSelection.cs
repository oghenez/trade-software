...
namespace baseClass.controls
{
    public partial class BizSectorTypesSelection : common.controls.baseUserControl
    {
        ...
        public AppTypes.BizSectorTypes myBizSectorType
        {
        ...
        {
            this.bizSectorTypesCb.LoadList(new AppTypes.BizSectorTypes[] 
                        { AppTypes.BizSectorTypes.None, 
                          AppTypes.BizSectorTypes.Industry, 
                          AppTypes.BizSectorTypes.SuperSector, 
                          AppTypes.BizSectorTypes.Sector}
                        );
        }
        public virtual void LockEdit(bool state)
        {
            bizSectorTypesCb.Enabled = !state;
            bizSectorCb.Enabled = (bizSectorTypesCb.myValue != AppTypes.BizSectorTypes.None) && !state;
        }
        private void LoadDataIndustry(ComboBox obj)
        {
            databases.baseDS.bizIndustryDataTable industryTbl = DataAccess.Libs.myBizIndustryTbl;
            obj.DataSource = null;
            obj.Items.Clear();
        ...
        {
            databases.baseDS.bizSectorDataTable sectorTbl = DataAccess.Libs.myBizSectorTbl;
            obj.DataSource = null;
            obj.Items.Clear();
        ...
        private void bizSectorTypesCb_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (bizSectorTypesCb.myValue)
                {
                    case AppTypes.BizSectorTypes.Industry: LoadDataIndustry(bizSectorCb); break;
                    case AppTypes.BizSectorTypes.SuperSector: LoadDataSuperSector(bizSectorCb); break;
                    case AppTypes.BizSectorTypes.Sector: LoadDataSector(bizSectorCb); break;
                }
                if (mySectorSelectionChange != null) mySectorSelectionChange(sender, e);
            }
            catch (Exception er)
