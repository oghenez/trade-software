...
{
    public partial class _makeIndicatorData : common.forms.baseForm
    {
        private bool fCanceled = false;
        public _makeIndicatorData()
        {
            try
            {
                InitializeComponent();
        ...
        private void MakeIndicatorData(myTypes.timeScales timeScale,DateTime frDate, DateTime toDate,StringCollection indicatorCode,StringCollection stockCode)
        {
            ...
            for (int count = 0; count < stockCode.Count; count++)
            {
                ...
                application.dataLibs.LoadData(myDataSet.priceData,timeScale,frPriceDate, toDate, stockCode[count]);
