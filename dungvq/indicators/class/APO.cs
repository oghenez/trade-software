...
namespace Indicators
{
    ...
    /// APO Helper class
    /// </summary>
    public class APOHelper : IndicatorHelper
    {
        public APOHelper()
        {
            Init(typeof(APO), typeof(forms.APO), Consts.constIndicatorMetaFile);
        }
    }
    ...
    public class APO : DataSeries
    {
        ...
        public static APO Series(DataSeries ds, int fastperiod, int slowperiod, int matype, string name)
        {
            ...
            if (ds.Cache.ContainsKey(description)) return (APO)ds.Cache[description];

            //Create aroon, cache it, return it
            APO apo = new APO(ds, fastperiod,slowperiod,matype, description);
            ds.Cache[description] = apo;
            return apo;
        }
