...
namespace test
{
    public partial class chartTest : baseClass.forms.baseForm
    {
        ...
        private int LoadDataLEFT()
        {
            ...
            application.DbAccess.LoadData(dataTbl, cbTimeScale.myValue.Code, frDate, toDate, codeEd.Text.Trim());
            if (dataTbl.Count > 0)
            {
                myData.AddDataToLeft(dataTbl);
        ...
        private void testBtn_Click(object sender, EventArgs e)
        {
            ...
            myData.DataTimeScale = cbTimeScale.myValue;
            myData.DataStockCode = codeEd.Text.Trim();
            application.AppLibs.LoadAnalysisData(myData);

            // Get the elapsed time as a TimeSpan value.
            TimeSpan ts = stopWatch.Elapsed;

            // Format and display the TimeSpan value.
            this.Text  = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",   ts.Hours, ts.Minutes, ts.Seconds, ts.Milliseconds / 10);
