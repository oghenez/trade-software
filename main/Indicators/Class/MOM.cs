...
namespace Indicators
{
    ...
    public class MOMHelper : Helpers
    {
        public MOMHelper()
        {
            Init(typeof(MOM), typeof(application.forms.commonIndicatorForm));
        }
    }
    ...
    public class MOM : DataSeries
    {
        ...
        /// <returns></returns>
        public static MOM Series(DataSeries ds, double period, string name)
        {
            //Build description
        ...
        /// <param name="name"></param>
        public MOM(DataSeries db, double period, string name)
            : base(db, name)
        {
            ...
            retCode = Core.Mom(0, db.Count - 1, db.Values, (int)period, out begin, out length, output);
